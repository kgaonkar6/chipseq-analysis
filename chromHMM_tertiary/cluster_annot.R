...
filename <- args[1]
plot_color<-c("red","blue","green","pink","yellow","black","orange")
clu_distance="correlation"
my_data=read.delim(filename, sep="\t", header=T)
### or whatever number you want
my_data[my_data<=0]<-1
#my_data
my_data_mat=as.matrix(my_data[,9:29])
top<-nrow(my_data)
top
rownames(my_data_mat)=paste(my_data[,1],my_data[,2],my_data[,3], sep="_")
colnames(my_data_mat)=c("366","389","392","393","394","396","398","399","400","402","406","408","409","410","412","413","414","416","417","419","420")
...
cancer_type<-data.frame(factor(c("Cirrhosis","Control","Cirrhosis","Control","Cirrhosis","Control","Cirrhosis","Cirrhosis","Control","Cirrhosis","SevereAH","SevereAH","SevereAH","SevereAH","SevereAH","SevereAH","SevereAH","SevereAH","Cirrhosis","Cirrhosis","Cirrhosis")))
...
hm.parameters <- list(my_final_data_mat,  
  ...
  cluster_rows = TRUE, cluster_cols = TRUE,clustering_distance_rows="euclidean",clustering_distance_cols="euclidean")
